...
## Faulcon DeLacy shop - Powered by Polymer Starter Kit
...
And that's it, the server is running and will be displayed in your web browser !

### Issues

We got several issue binding data after they were initialized in the page, especially for the paintjob page. We have all the material to display paintjob for each ships,
however the update of the card's URL didn't goes that well, forcing us to only display one set of paintjob and pretending it will fit for every ships.

Another issue we encounter is the gesture of picture's size, for some category we didn't have a standard size so it leads to card with different size. We apologize for it.
